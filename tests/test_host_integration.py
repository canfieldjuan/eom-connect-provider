...
_ISSUED_LINKS = {
    "success": True,
    "links": [{"draftId": "aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa"}],
    "limit": 10,
    "cursor": None,
    "hasMore": False,
    "nextCursor": None,
...
def test_host_discovers_and_invokes_issued_links_read(tmp_path):
    tracker = StubTracker.start()
    private_key = Ed25519PrivateKey.generate()
    device_id = str(uuid4())
    tracker.register_public_key(
    ...
    tracker.set_issued_links(dict(_ISSUED_LINKS))
    client = TrackerClient(tracker.base_url, store.DeviceCredential(device_id, private_key))
    ...
    try:
        ...
        capability = items[capabilities.PUBLIC_LINK_LIST_CAPABILITY_ID]
        assert capability.confirmation_required is False

        content = b""
        job = connect.prepare_capability_job(
            capability, content, "application/json", "query.json", parameters={"limit": 10}
        )
        completed = connect.ConnectV2Client(capability).submit(job, content)
        assert completed.status == "completed"
        assert completed.result is not None
        ...
        assert json.loads(output.payload) == _ISSUED_LINKS
    finally:
        provider.stop()
        tracker.stop()
...
def test_host_invokes_mark_working_with_confirmation(tmp_path):
    tracker = StubTracker.start()
    private_key = Ed25519PrivateKey.generate()
    device_id = str(uuid4())
    tracker.register_public_key(
    ...
    try:
        ...
        capability = items[capabilities.MARK_WORKING_CAPABILITY_ID]
        assert capability.confirmation_required is True

        content = json.dumps(
            {
                "contactId": _DRAFT_ID,
                "expectedStateToken": "a" * 64,
                "confirmationId": _CONFIRMATION_ID,
            }
        ).encode()
        ...
        with pytest.raises(connect.ConnectError):
            connect.prepare_capability_job(capability, content, media_type, "mark-working.json")

        job = connect.prepare_capability_job(
            capability, content, media_type, "mark-working.json", confirmed=True
        )
        completed = connect.ConnectV2Client(capability).submit(job, content)
        assert completed.status == "completed"
        assert completed.result is not None
        ...
        receipt = json.loads(output.payload)
        assert receipt["workingLead"]["contactId"] == _DRAFT_ID
        assert len(tracker.state.minted_challenges) == 1
    finally:
        provider.stop()
        tracker.stop()
...
def test_host_invokes_public_link_revoke_with_confirmation(tmp_path):
    tracker = StubTracker.start()
    private_key = Ed25519PrivateKey.generate()
    device_id = str(uuid4())
    tracker.register_public_key(
    ...
    try:
        ...
        capability = items[capabilities.PUBLIC_LINK_REVOKE_CAPABILITY_ID]
        assert capability.confirmation_required is True
        ...
        with pytest.raises(connect.ConnectError):
            connect.prepare_capability_job(capability, content, media_type, "revocation.json")

        job = connect.prepare_capability_job(
            capability, content, media_type, "revocation.json", confirmed=True
        )
        completed = connect.ConnectV2Client(capability).submit(job, content)
        assert completed.status == "completed"
        assert completed.result is not None
